...
export function App() {
  ...
  const bootstrap = useQuery({
    ...
    queryFn: async () => successful<Bootstrap>(await getBootstrap()),
  });
  return (
    <div className="studio-shell">
      ...
      <main>
        <header className="topbar">
          <div>
            <p className="eyebrow">Kick webhook workbench</p>
            <h1>{page}</h1>
          </div>
          <div className="workspace-chip">
            ...
            <span>
              ...
              <strong>{bootstrap.data?.workspace.path ?? "Loading…"}</strong>
            </span>
          </div>
        </header>
