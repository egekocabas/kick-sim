      ...
      if (url === "/api/bootstrap")
        return response({
          ...
          workspace: {
            ...
            destinationUrl: "http://127.0.0.1:3000/webhooks/kick",
            historyEnabled: true,
            databasePath: "/tmp/.kick-sim/.runtime/kick-sim.db",
          },
          key: {
            algorithm: "RSA",
            bits: 2048,
            fingerprint: "test",
            matchingPrivateKey: true,
...
describe("scenario source editor", () => {
  ...
  it("warns about an external revision without replacing the open draft", async () => {
    ...
    await act(async () => {
      await client.invalidateQueries({ queryKey: ["scenario-source", custom.id] });
    });
